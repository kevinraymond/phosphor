...
- **README and gallery media regenerated** — a new hero montage and a clip of every one of the 38 effects, replacing a five-month-old teaser and a UI screenshot that existed only as a remote URL, so it broke in the release tarball. Splat's clips were re-shot after the renderer fixes below, so the published ones are no longer mirrored.
