...
### Added
- **Strata — spectral-canyon effect (first pass)** — a heightfield raymarcher flown forward over the A17 `spectrogram()` mel-history: the last ~8 seconds of audio become terrain (loud moments = ridges, quiet = chasms), lateral position = frequency band (palette-coloured), scrolling toward the camera with the music. The second A17 texture consumer, and the first consumer of `spectrogram()` and `u.rolloff`. Audio: `rolloff` → draw distance / fog, `flatness` → surface gloss, `beat_phase` → camera bob, `kick` → brightness, `beat` → wet specular. Renders end-to-end; deeper visual polish (anti-aliasing/LOD, temporal smoothing, materials, soft shadows) is tracked as board #1508. (board #1479)
