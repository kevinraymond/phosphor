...
- Particle SoA layout: 4 separate `vec4f` storage buffers (`pos_life`, `vel_size`, `color`, `flags`), 16 bytes each, 64 bytes total per particle. Position-only reads (spatial hash, sort keygen) load 16B instead of 64B — 4× bandwidth savings. `read_particle()`/`write_particle()` helpers abstract the multi-buffer access. `pos_life.z` stores spin angle (accumulated each frame when `spin_speed != 0`); chaos_sim uses it for 3D depth (spin gated by spin_speed check).
